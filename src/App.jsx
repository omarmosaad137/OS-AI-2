...
const initialLead = {
  ...
  phone: '',
  source: 'Website',
  matter_type: 'Commercial / Civil Recovery',
  forum: 'Dubai Courts',
  opponent: '',
  stage: 'New Enquiry',
  priority: 'Normal',
...
export default function App() {
  ...
  const [aiPrompt, setAiPrompt] = useState('Draft a professional client update using the selected matter.');
  ...
  async function loadProfile(userId) {
    const { data, error } = await supabase
      .from('profiles')
      .select('user_id, email, full_name, avatar_url, role, status, client_id, clients(name, email, phone)')
      .eq('user_id', userId)
      .maybeSingle();
  ...
  async function loadLeads() {
    ...
    const { data, error } = await supabase
      .from('leads')
      .select('id, name, email, phone, source, matter_type, forum, opponent, stage, priority, assigned_to, followup_date, facts, notes, created_at')
      .order('created_at', { ascending: false });
  ...
  async function loadMatters() {
    const { data, error } = await supabase
      .from('matters')
      .select('id, ref, title, client_id, matter_type, forum, opponent, stage, status, deadline, facts, next_step, clients(name, email, phone)')
      .order('created_at', { ascending: false });
  ...
  async function createLead(event) {
    event.preventDefault();
    setMessage('');
    if (!newLead.name.trim()) { setMessage('Lead name is required.'); return; }
    const { error } = await supabase.from('leads').insert({
      ...
      forum: newLead.forum,
      opponent: newLead.opponent || null,
      stage: newLead.stage,
      priority: newLead.priority,
      assigned_to: newLead.assigned_to || null,
      followup_date: newLead.followup_date || null,
      facts: newLead.facts || null,
  ...
  async function convertLeadToMatter(lead) {
    setMessage('');
    let clientId = null;
    const { data: existingClient } = await supabase.from('clients').select('id').eq('email', lead.email || '').maybeSingle();
    ...
    if (!clientId) {
      ...
      if (clientError) { setMessage(clientError.message); return; }
      clientId = client.id;
    }
    const ref = `OSL-${new Date().getFullYear()}-${String(matters.length + 1).padStart(4,'0')}`;
    const { error: matterError } = await supabase.from('matters').insert({
      ref,
      title: `${lead.matter_type} — ${lead.name}`,
      client_id: clientId,
      matter_type: lead.matter_type,
      forum: lead.forum,
      opponent: lead.opponent || null,
      stage: 'New Enquiry',
      status: 'Active',
      deadline: lead.followup_date || null,
      facts: lead.facts || null,
      next_step: 'Complete conflict check, KYC and engagement process.'
    });
    if (matterError) { setMessage(matterError.message); return; }
    await supabase.from('leads').update({ stage: 'Converted' }).eq('id', lead.id);
    await loadDashboardData();
    setPage('matters');
  }

  async function updateMatter(id, patch) {
    setMessage('');
  ...
  function applyEmailTemplate(templateId, matterId) {
    ...
    const fill = (text) => String(text || '').replace(/\{([a-z_]+)\}/g, (_, key) => map[key] ?? '');
    setNewEmailDraft({
      ...newEmailDraft,
      matter_id: matter.id,
      to_email: matter.clients?.email || '',
      subject: fill(template.title),
      body: fill(template.body),
      email_type: template.category || 'Client update'
    });
  }

  async function generateAiDraft() {
    const matter = matters.find((item) => item.id === newEmailDraft.matter_id) || matters[0];
    const context = matter ? [
      `Matter: ${matter.ref} — ${matter.title}`,
      `Client: ${matter.clients?.name || ''}`,
      `Opponent: ${matter.opponent || ''}`,
      `Stage: ${matter.stage || ''}`,
      `Facts: ${matter.facts || ''}`,
      `Next step: ${matter.next_step || ''}`
    ].join('\n') : '';
    try {
      const response = await fetch('/api/ai', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode: 'email_template', prompt: aiPrompt, context })
      });
      const result = await response.json();
      const text = result.text || 'No AI text returned.';
      setAiOutput(text);
      setNewEmailDraft({ ...newEmailDraft, body: text, subject: newEmailDraft.subject || `Update regarding ${matter?.ref || ''}` });
    } catch (e) {
      setAiOutput('AI route is not configured yet. Add /api/ai and AI provider variables.');
    }
  }
    ...
    <div className="appShell">
      ...
      <main className="main">
        ...
        <section className="content">
          ...
          {page === 'leads' && isManager && (
            <div className="grid two">
              <Card title="Create Lead">
                <form onSubmit={createLead} className="formGrid">
                  <Field label="Name"><input value={newLead.name} onChange={(e)=>setNewLead({...newLead,name:e.target.value})} /></Field>
                  <Field label="Email"><input value={newLead.email} onChange={(e)=>setNewLead({...newLead,email:e.target.value})} /></Field>
                  <Field label="Phone"><input value={newLead.phone} onChange={(e)=>setNewLead({...newLead,phone:e.target.value})} /></Field>
                  <Field label="Source"><select value={newLead.source} onChange={(e)=>setNewLead({...newLead,source:e.target.value})}>{['Website','WhatsApp','Referral','Existing Client','Online Platform','Walk-in'].map(x=><option key={x}>{x}</option>)}</select></Field>
                  <Field label="Matter type"><input value={newLead.matter_type} onChange={(e)=>setNewLead({...newLead,matter_type:e.target.value})} /></Field>
                  <Field label="Forum"><input value={newLead.forum} onChange={(e)=>setNewLead({...newLead,forum:e.target.value})} /></Field>
                  <Field label="Opponent"><input value={newLead.opponent} onChange={(e)=>setNewLead({...newLead,opponent:e.target.value})} /></Field>
                  <Field label="Follow-up"><input type="date" value={newLead.followup_date} onChange={(e)=>setNewLead({...newLead,followup_date:e.target.value})} /></Field>
                  <Field label="Facts"><textarea value={newLead.facts} onChange={(e)=>setNewLead({...newLead,facts:e.target.value})} /></Field>
                  <button className="primary">Create Lead</button>
                </form>
              </Card>
              <Card title="Leads Pipeline">
                <table><thead><tr><th>Name</th><th>Type</th><th>Stage</th><th>Follow-up</th><th>Action</th></tr></thead><tbody>
                {leads.map(lead=><tr key={lead.id}><td><strong>{lead.name}</strong><br/><small>{lead.email || lead.phone}</small></td><td>{lead.matter_type}</td><td><Pill>{lead.stage}</Pill></td><td>{lead.followup_date || '-'}</td><td>{lead.stage !== 'Converted' && <button onClick={()=>convertLeadToMatter(lead)}>Convert</button>}</td></tr>)}
                </tbody></table>
              </Card>
            </div>
          )}
          ...
          {page === 'matters' && (
            <div className="grid two wide-first">
              <Card title={isClient ? 'My Matters' : 'Matters'}>
                <table>
                  <thead>
                    <tr><th>Reference</th><th>Title</th><th>Client</th><th>Stage</th><th>Status</th></tr>
                  </thead>
                  <tbody>
                    {matters.map((matter) => (
                      <tr key={matter.id} onClick={() => setSelectedMatterId(matter.id)} className={selectedMatterId === matter.id ? 'selected-row' : ''}>
                        <td><strong>{matter.ref}</strong></td>
                        <td>{matter.title}</td>
                        <td>{matter.clients?.name || '-'}</td>
                        <td>{matter.stage}</td>
                        <td><Pill>{matter.status}</Pill></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <Card title="Matter Workspace / Litigation Practice">
                {(() => {
                  const matter = matters.find((item) => item.id === selectedMatterId) || matters[0];
                  if (!matter) return <p>No matter selected.</p>;
                  return (
                    <div className="formGrid">
                      <Field label="Reference"><input value={matter.ref || ''} onChange={(e) => updateMatter(matter.id, { ref: e.target.value })} /></Field>
                      <Field label="Title"><input value={matter.title || ''} onChange={(e) => updateMatter(matter.id, { title: e.target.value })} /></Field>
                      <Field label="Forum / Court"><input value={matter.forum || ''} onChange={(e) => updateMatter(matter.id, { forum: e.target.value })} /></Field>
                      <Field label="Opponent"><input value={matter.opponent || ''} onChange={(e) => updateMatter(matter.id, { opponent: e.target.value })} /></Field>
                      <Field label="Stage">
                        <select value={matter.stage || 'New Enquiry'} onChange={(e) => updateMatter(matter.id, { stage: e.target.value })}>
                          {['New Enquiry','Conflict Check','Legal Notice','Filing / Registration','Service / Notification','Hearing / Investigation','Expert / Evidence','Judgment / Award','Appeal / Cassation','Execution / Recovery','Closed'].map(x => <option key={x}>{x}</option>)}
                        </select>
                      </Field>
                      <Field label="Status">
                        <select value={matter.status || 'Active'} onChange={(e) => updateMatter(matter.id, { status: e.target.value })}>
                          {['Active','Waiting Client','Waiting Court','On Hold','Closed'].map(x => <option key={x}>{x}</option>)}
                        </select>
                      </Field>
                      <Field label="Deadline"><input type="date" value={matter.deadline || ''} onChange={(e) => updateMatter(matter.id, { deadline: e.target.value || null })} /></Field>
                      <Field label="Facts / Case Background"><textarea value={matter.facts || ''} onChange={(e) => updateMatter(matter.id, { facts: e.target.value })} /></Field>
                      <Field label="Next Step / Litigation Strategy"><textarea value={matter.next_step || ''} onChange={(e) => updateMatter(matter.id, { next_step: e.target.value })} /></Field>
                      <div className="practice-box">
                        <strong>Litigation practice checklist</strong>
                        <ul>
                          <li>Conflict check completed</li>
                          <li>KYC / POA / authority documents checked</li>
                          <li>Evidence index prepared</li>
                          <li>Court fee / expert fee requested if needed</li>
                          <li>Hearing date and preparation note entered</li>
                          <li>Client update drafted after each procedural step</li>
                        </ul>
                      </div>
                    </div>
                  );
          ...
          {page === 'email' && canUseInternalComms && (
            <div className="grid two">
              <Card title="Email Service + Templates + AI Drafting">
                <div className="actions">
                  ...
                  <select value={selectedEmailTemplateId} onChange={(event)=>{ setSelectedEmailTemplateId(event.target.value); applyEmailTemplate(event.target.value, newEmailDraft.matter_id); }}>
                    <option value="">Select template from Library</option>
                    {libraryTemplates.filter(t => ['Client Update','Legal Notice','Payment Reminder','Document Request','Engagement Letter'].includes(t.category)).map(t => <option key={t.id} value={t.id}>{t.title} — {t.category}</option>)}
                  </select>
                  <button type="button" onClick={() => buildClientUpdateEmail(newEmailDraft.matter_id)}>Matter Update</button>
                </div>
                <form onSubmit={createEmailDraft} className="formGrid">
                  <Field label="Email type"><select value={newEmailDraft.email_type} onChange={(e)=>setNewEmailDraft({...newEmailDraft,email_type:e.target.value})}>{['Client update','Payment reminder','Document request','Legal notice cover','Internal notification','General'].map(x=><option key={x}>{x}</option>)}</select></Field>
                  <Field label="To"><input value={newEmailDraft.to_email} onChange={(e)=>setNewEmailDraft({...newEmailDraft,to_email:e.target.value})} /></Field>
                  <Field label="Subject"><input value={newEmailDraft.subject} onChange={(e)=>setNewEmailDraft({...newEmailDraft,subject:e.target.value})} /></Field>
                  <Field label="Body"><textarea value={newEmailDraft.body} onChange={(e)=>setNewEmailDraft({...newEmailDraft,body:e.target.value})} /></Field>
                  <Field label="AI smart prompt"><textarea value={aiPrompt} onChange={(e)=>setAiPrompt(e.target.value)} /></Field>
                  <div className="actions"><button type="button" onClick={generateAiDraft}>Generate AI Draft</button><button className="primary">Save Email Draft</button></div>
                </form>
                {aiOutput && <pre className="ai-output">{aiOutput}</pre>}
              </Card>
              <Card title="Email Queue">
                <table><thead><tr><th>To</th><th>Subject</th><th>Matter</th><th>Status</th><th>Action</th></tr></thead><tbody>
                  {emailQueue.map((email)=><tr key={email.id}><td>{email.to_email}</td><td><strong>{email.subject}</strong><br/><small>{email.email_type}</small></td><td>{email.matters?.ref || '-'}</td><td><Pill>{email.status}</Pill></td><td><a className="button" href={`mailto:${encodeURIComponent(email.to_email)}?subject=${encodeURIComponent(email.subject)}&body=${encodeURIComponent(email.body)}`}>Open</a>{email.status !== 'sent' && <button onClick={()=>sendQueuedEmail(email)}>Send</button>}</td></tr>)}
                </tbody></table>
              </Card>
            </div>
          )}
            ...
            <div className="grid two">
              <Card title="System Settings">
                <div className="practice-box">
                  ...
                  <ul>
                    <li>Control users and roles from Users page</li>
                    <li>Control announcements from Announcements page</li>
                    <li>Control library templates from Library page</li>
                    <li>Control finance approvals from Finance Operations</li>
                    <li>Control HR/Payroll from HR System</li>
                  </ul>
                </div>
              </Card>
              <Card title="Profile Photo">
                <Field label="Avatar URL"><input defaultValue={profile.avatar_url || ''} onBlur={(e)=>supabase.from('profiles').update({avatar_url:e.target.value}).eq('user_id', profile.user_id).then(()=>loadProfile(profile.user_id))} /></Field>
                <p>Paste a public image URL and click outside the field to save.</p>
              </Card>
            </div>
          )}
